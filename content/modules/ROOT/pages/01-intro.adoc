# What is Podman? If you are not already familiar with podman, let us
...
# What is a pod? Additionally you may be wondering what a pod is within
...
# And what then, is Quadlet? Quadlet is a generator for systemd that
will create unit files to control podman resources through systemctl.
...
back-end.
`+bash,run   podman run -d --pod my-app --name database -e MYSQL_ROOT_PASSWORD="r3dh@t123" -e MYSQL_USER="dbuser" -e MYSQL_PASSWORD="redhat" -e MYSQL_DATABASE="testdb" quay.io/fedora/mariadb-105+`
